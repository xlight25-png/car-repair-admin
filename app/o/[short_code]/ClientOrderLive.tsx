...
export default function ClientOrderLive({
  ...
  const lastMediaFetchAt = useRef<number>(0);
  ...
  const loadMedia = useCallback(async () => {
    // лёгкий анти-спам (если много апдейтов подряд)
    const now = Date.now();
    if (now - lastMediaFetchAt.current < 600) return;
    lastMediaFetchAt.current = now;
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setOrder((prev) => {
            const nextOrder = { ...prev, ...next };

            // ✅ если updated_at изменился — перетянем медиа (добавили/удалили/обновили)
            if ((prev.updated_at ?? null) !== (nextOrder.updated_at ?? null)) {
              // не блокируем setState
              queueMicrotask(() => loadMedia());
            }
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl">
          ...
          <div className="mt-1 text-slate-300">
            Госномер: <span className="font-mono">{order.vehicle.license_plate}</span>
          </div>
